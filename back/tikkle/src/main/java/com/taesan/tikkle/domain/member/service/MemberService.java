...
public class MemberService {
    ...
    public AllMemberBoardResponse getMemberBoard(UUID username) {
        if (memberRepository.findById(username).isEmpty()) throw new CustomException(ErrorCode.MEMBER_NOT_FOUND);
        ...
        for(Board board : boards){
            MemberBoardResponse response = new MemberBoardResponse(board.getId(),board.getTitle(),board.getContent(),board.getStatus(),board.getTime(),board.getCreatedAt());
            switch (response.getStatus()){
                case "POSTED"-> postedPosts.add(response);
                case "ACTIVE" -> activePosts.add(response);
                case "DONE" -> donePosts.add(response);
            }
        }
