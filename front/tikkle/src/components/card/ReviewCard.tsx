...
import { useCreateReview } from "@/hooks"; // useCreateReview 훅 사용
import { ReviewCreateRequest } from "@/types"; // ReviewCreateRequest 타입 사용

const ReviewCard: React.FC = () => {
  const [selectedBadge, setSelectedBadge] = useState<string | null>(null); // 선택된 리뷰 타입
  const [reviewContent, setReviewContent] = useState<string>(""); // 리뷰 내용

  const { mutate: createReviewMutate, isSuccess, isError } = useCreateReview();
